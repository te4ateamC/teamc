...
public class OrderController {
    ...
    public String showReserveApproPage(@ModelAttribute Reservation reservation, Model model) {
        model.addAttribute("reservation", reservation);
        return "Reserveappro"; // Reserveappro.htmlを表示
    }

    @PostMapping("/complete")
    public String complete(@RequestParam Map<String, String> params) {
        reservationService.complete(params);
        return "redirect:/orderhistory"; // ← ここも修正
    }
    ...
    public String showOrderHistory(Model model) {
        List<Reservation> reservationList = reservationService.findAll();
        model.addAttribute("reservationList", reservationList);
        return "orderhistory"; // orderhistory.html に対応する Thymeleaf テンプレート
    }
